...
def df_orders_valido():
    """DataFrame de orders que cumple todas las reglas de SCHEMA_ORDERS."""
    return pd.DataFrame(
        {
            "order_id": ["a" * 32, "b" * 32],
...
def df_orders_status_invalido():
    """DataFrame de orders con order_status fuera del conjunto permitido."""
    return pd.DataFrame(
        {
            "order_id": ["a" * 32],
...
def df_items_precio_cero():
    """DataFrame de items con price = 0.0, que viola price > 0."""
    return pd.DataFrame(
        {
            "order_id": ["a" * 32],
            "order_item_id": [1],
